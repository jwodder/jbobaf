...
if (@ARGV) {
 foreach (@ARGV) {
  ...
  if ($opts{e}) {
   ...
   @valsi = grep { $_->isCmavo() } @valsi if ($opts{c} || $opts{C}) &&
    ($opts{r} || $opts{R});
  }
  @valsi = grep { $_->isGismu() } @valsi if $opts{G};
  @valsi = grep { !$_->isCompoundCmavo() } @valsi if $opts{C};
...
Some I<cmavo> which appear in both the I<gismu> list and the I<cmavo> list have
different definitions in each, and currently the best way to handle them is to
concatenate them with "C< --- >".  This can generate odd-looking results,
especially for I<cmavo> whose two definitions differ only in a single word.
